...
interface Hotel {
	 ...
	 * 
	 * 
	 * Que - Why we need the nested interfaces? 
	 * Ans - Because we dont need to show/give all method to classes. or we just put some special methods for some
	 * specific kind of purposes
	 * 
	 * 
	 * We do this to group methods together. Any class who wants these methods will
...
/*
 * Here we are implements the nested interfaces by the " . " operator and we can
 * access all methods that are presented inside the interfaces are used
 */

/*
 * Below we have class SunAndSand we are implementing both the outer and inner
